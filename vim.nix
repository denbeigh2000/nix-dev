...
let
  ...
  allPlugins = vimPlugins:
    (pythonPlugins vimPlugins) ++
    (rustPlugins vimPlugins) ++
    (goPlugins vimPlugins) ++
    (jsPlugins vimPlugins) ++
    (basePlugins vimPlugins);

  nvimCustom = mkPlugins: with pkgs; neovim.override {
    configure = {
      customRC = ''
      ...
      packages.myVimPackage = {
        start = (basePlugins pkgs.vimPlugins) ++ (mkPlugins pkgs.vimPlugins);
      };
    };
  };
in
